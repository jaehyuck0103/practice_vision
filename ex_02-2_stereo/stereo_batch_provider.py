...
class KITTIDataset:
    def __init__(self, image_dir, disparity_dir=None, downsample=True):
        ...
        print(f"KITTI data loaded ({len(self._left_images)} images)!")
    ...
    def __getitem__(self, i):
        ...
        if self.downsample:
            img_l = rescale(img_l, 0.5, mode="reflect", anti_aliasing=True, channel_axis=2)
            img_r = rescale(img_r, 0.5, mode="reflect", anti_aliasing=True, channel_axis=2)
...
class PatchProvider:
    ...
    def iterate_batches(self, batch_size):
        ...
        ref_batch = np.zeros(
            (self._cache * batch_size,) + self._patch_size + (channels,), dtype="float32"
        )
        pos_batch = np.zeros_like(ref_batch)
        neg_batch = np.zeros_like(ref_batch)

        # start the thread
        self._thread = Thread(target=self.fill_batches, args=(ref_batch, pos_batch, neg_batch))
        self._stop = False
        self._thread.start()
        ...
        while True:
            ...
            with self._lock:
                yield torch.Tensor(ref_batch[self.idxs]).cuda(), torch.Tensor(
                    pos_batch[self.idxs]
                ).cuda(), torch.Tensor(neg_batch[self.idxs]).cuda()
